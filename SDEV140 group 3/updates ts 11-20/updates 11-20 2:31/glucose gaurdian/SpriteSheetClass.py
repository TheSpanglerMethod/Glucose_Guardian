...
class SpriteSheet(object):
    ...
    def get_image_v2(self, x,y, width,height):
        ...
        image.set_colorkey((0,0,0))#whatever color this is set to is removed from the image displayed
        return image
    ...
    def get_animate(self,frames_to_get,startX,startY,width,height,VerticalFlip, HorizontalFlip):
        ...
        for i in range(frames_to_get):
            image = self.get_image_v2(x,y,width,height)
            self.frames.append(pygame.transform.flip(image, VerticalFlip, HorizontalFlip))#this is where and how it flip flops images for lef/right facing
            x=x+width
        self.counter= 0
